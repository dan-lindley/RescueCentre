...
$stmt = $pdo->prepare('SELECT COUNT(*) FROM accounts WHERE id = ? AND role = "Developer"');
// Get the account info using the logged-in session ID
$stmt->execute([ $_SESSION['account_id'] ]);
// If the account exists with the specified ID and is an admin...
if ($stmt->fetchColumn() == 0) {
    exit('You do not have permission to access to the admin page!');
}
// Add/remove roles from the list
$roles_list = ['Admin', 'Member', 'Vet', 'Volunteer', 'Driver'];
...
<a href="patients.php"' . ($selected == 'patients' ? ' class="selected"' : '') . ' title="Patients">
    <span class="icon">
        <!-- You can use any Font Awesome SVG icon you want -->
        <svg width="15" height="15" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512">
            <path d="M224 256A128 128 0 1 0 224 0a128 128 0 1 0 0 256zM96 448a96 96 0 0 1 96-96h192a96 96 0 0 1 96 96v32H96V448z"/>
        </svg>
    </span>
    <span class="txt">Patients</span>
</a>
<div class="sub">
    <a href="patients.php"' . ($selected == 'patients' && $selected_child == 'view' ? ' class="selected"' : '') . '>
        <span class="square"></span>View Patients
    </a>
    <a href="patient_add.php"' . ($selected == 'patients' && $selected_child == 'add' ? ' class="selected"' : '') . '>
        <span class="square"></span>Add Patient
    </a>
    <a href="patient_admit.php"' . ($selected == 'patients' && $selected_child == 'admit' ? ' class="selected"' : '') . '>
        <span class="square"></span>Admit Patient
    </a>
</div>
...
<html lang="en">
    ...
    <body class="admin">
        <aside>
            ...
            ' . $admin_links . '
            <div class="footer">
                <a href="https://codeshack.io/package/php/advanced-secure-login-registration-system/" target="_blank">Rescue Centre</a>
                Version 2.0.0 <BR>Developer Admin
            </div>
        </aside>
